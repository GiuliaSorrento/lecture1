...
class Controller:
    ...
    def add_ordine(self, e):
        ...
        if nomePstr == "":
            self._view._lvOut.controls.append(
                ft.Text("Attenzione! il campo prodotto non può essere vuoto",
                        color="red")
            )
    ...
    def gestisci_ordine(self, e):   #invoca il metodo processa prossimo ordine
        self._view._lvOut.controls.clear()  #svuotiamo interfaccia
        res, ordine = self._model.processa_prossimo_ordine()
        if res:
            self._view._lvOut.controls.append(
                ft.Text(value="ordine processato con successo.", color = "green")
            )
    ...
    def gestisci_all_ordini(self, e):
        ...
        if not ordini: #se ordini è vuoto
            self._view._lvOut.controls.append(
                ft.Text(value="non ci sono ordini in coda", color = "blue")
            )
            ...
            self._view._lvOut.controls.append(
                ft.Text(value=f"ho processato correttamente {len(ordini)} di ordini", color ="green")
            )
            for o in ordini:
                self._view._lvOut.controls.append(ft.Text("\n"))
                self._view._lvOut.controls.append(
